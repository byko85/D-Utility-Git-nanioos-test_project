...
from step2_train import UNetAutoencoder


def get_anomaly_score(error_map, mode="top01_mean"):
...
def evaluate_performance(model, test_loader, device):
    ...
    print("현재 anomaly score mode: top01_mean / blur: (5,5)")
    ...
    with torch.no_grad():
        for images, labels, _ in test_loader:
            ...
            error = torch.mean(torch.abs(images - outputs), dim=1)
            error_map = error.squeeze().cpu().numpy()
            error_map = cv2.GaussianBlur(error_map, (5, 5), 0)

            anomaly_score = get_anomaly_score(error_map, mode="top01_mean")
...
if __name__ == "__main__":
    ROOT_DIR = "./mvtec_ad"
    CATEGORY = "bottle"
    MODEL_PATH = "unet_dae_bottle_best.pth"
